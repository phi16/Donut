...
pub fn item_to_kind(item: &env::Item) -> EntryKind {
    ty_to_kind(&item.ty)
}
...
pub(crate) fn build_entry_info(def_id: DefId, env: &Env, flat: &FlatEnv) -> EntryInfo {
...
pub(crate) fn build_module_info() -> EntryInfo {
    EntryInfo {
...
pub fn analyze(code: &str) -> AnalysisResult {
    ...
    // tokenizer の型付けを初期値として TokenData を構築 (UTF-8)
    ...
    let comment_tokens = comments.into_iter().map(|pos| TokenData {
